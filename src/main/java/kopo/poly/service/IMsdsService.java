...
public interface IMsdsService {
    ...
    MsdsDocumentResponseDTO attach(MsdsAttachRequestDTO request, Long createdBy);

    // 특정 점검에 첨부된 MSDS 목록.
    List<MsdsDocumentResponseDTO> findByInspectionId(Long inspectionId);

    // 내가 저장한 MSDS 전체 목록(독립 MSDS 조회 화면).
    List<MsdsDocumentResponseDTO> findMine(Long createdBy);

    // 저장된 MSDS 확인 상태 토글(현장관리자가 "이 물질/문서가 맞다"고 확인).
    MsdsDocumentResponseDTO setVerified(Long msdsId, boolean verified);
}
