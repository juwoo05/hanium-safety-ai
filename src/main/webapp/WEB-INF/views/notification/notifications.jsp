      ...
      <div class="lg:col-span-2 space-y-3">
        ...
        <c:forEach var="n" items="${notifications}">
          ...
          <a href="${pageContext.request.contextPath}/notifications/${n.id}/redirect"
             class="relative group block bg-white rounded-xl border transition-all hover:shadow-md
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-start justify-between gap-2 mb-1">
                  ...
                  <div class="flex items-center gap-1 flex-shrink-0">
                    ...
                    <c:if test="${!n.read}"><div class="w-2 h-2 rounded-full bg-[#FF6B35] flex-shrink-0"></div></c:if>
                  </div>
                </div>
      ...
      <div class="space-y-5">
        ...
        <div class="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <h2 class="text-sm font-semibold text-gray-900 mb-4 flex items-center gap-2">
            <svg class="w-4 h-4 text-[#FF6B35]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
            마감 일정
          </h2>
          <div id="deadlineGroups" class="space-y-5">
            <p class="text-sm text-gray-400">불러오는 중...</p>
          </div>
          <a href="/actions" class="w-full mt-4 py-2.5 bg-[#FF6B35] text-white rounded-lg text-sm font-semibold hover:bg-[#E55A2A] transition-colors flex items-center justify-center gap-2">
        ...
        qs('#deadlineGroups').innerHTML = order.slice(0, 4).map(function (date) {
          ...
          var items = groups[date].slice(0, 4).map(function (a) {
            ...
            return '<a href="' + link + '" class="flex items-center gap-2.5 p-2.5 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors group">' +
              ...
              '<svg class="w-3 h-3 text-gray-300 group-hover:text-[#FF6B35] transition-colors flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path d="M5 12h14M12 5l7 7-7 7"/></svg></a>';
          }).join('');
  ...
  function renderSettings() {
    ...
    qs('#settingsList').innerHTML = SETTINGS_DEFS.map(function (s) {
      ...
      return '<label class="flex items-center justify-between cursor-pointer" data-key="' + s.key + '">' +
        ...
        '<div class="toggle relative w-10 h-5 rounded-full transition-colors flex-shrink-0 ' + (on ? 'bg-[#FF6B35]' : 'bg-gray-200') + '">' +
