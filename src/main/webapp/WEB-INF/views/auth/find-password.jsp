...
<div class="bg-white rounded shadow-2xl p-10 w-full max-w-md">
  ...
  <div class="flex items-center gap-2 mb-8">
    <c:forEach var="i" begin="1" end="4">
      ...
      <div class="flex flex-col items-center gap-1">
        <div class="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${i <= step ? 'bg-[#1A2E44] text-white' : 'bg-gray-200 text-gray-400'}">${i}</div>
        <span class="text-xs ${i <= step ? 'text-[#1A2E44]' : 'text-gray-400'}">
          <c:choose>
  ...
  <c:if test="${not empty findPwError}">
    <div class="bg-red-50 border border-red-200 text-red-600 text-sm rounded px-4 py-3 mb-5">${findPwError}</div>
  </c:if>
    ...
    <form action="/find-password/send-code" method="post">
      ...
      <div class="relative mb-4">
        ...
        <input type="email" name="email" placeholder="가입 시 등록한 이메일" required
               class="w-full pl-10 pr-4 py-3 border border-gray-300 rounded focus:ring-2 focus:ring-[#1A2E44] focus:border-transparent outline-none"/>
      </div>
      <button type="submit" class="w-full bg-[#1A2E44] text-white py-3 rounded font-semibold hover:bg-[#0F2233] transition-colors">인증 코드 발송</button>
    </form>
  </c:if>
  ...
  <c:if test="${step == 2}">
    <form action="/find-password/verify-code" method="post">
      ...
      <input type="text" name="code" placeholder="인증 코드 6자리" maxlength="6" required inputmode="numeric"
             class="w-full px-4 py-3 border border-gray-300 rounded focus:ring-2 focus:ring-[#1A2E44] focus:border-transparent outline-none mb-4"/>
      <button type="submit" class="w-full bg-[#1A2E44] text-white py-3 rounded font-semibold hover:bg-[#0F2233] transition-colors">코드 확인</button>
    </form>
  </c:if>
  ...
  <c:if test="${step == 3}">
    <form action="/find-password/reset" method="post" onsubmit="return validateNewPassword()">
      ...
      <button type="submit" class="w-full bg-[#1A2E44] text-white py-3 rounded font-semibold hover:bg-[#0F2233] transition-colors">비밀번호 변경</button>
    </form>
  </c:if>
  ...
  <c:if test="${step == 4}">
    <div class="text-center">
      ...
      <a href="/login" class="block w-full bg-[#1A2E44] text-white py-3 rounded font-semibold hover:bg-[#0F2233] transition-colors text-center">로그인하기</a>
    </div>
  </c:if>
</div>
