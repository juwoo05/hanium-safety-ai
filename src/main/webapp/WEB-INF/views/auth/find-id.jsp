...
<div class="bg-white rounded-3xl shadow-2xl p-10 w-full max-w-md">
  ...
  <div class="flex items-center gap-2 mb-8">
    <div class="w-12 h-12 bg-[#1B3A5F] rounded-xl flex items-center justify-center">
    ...
    <div>
      <span class="font-bold text-2xl text-[#1B3A5F]">아이디 찾기</span>
  ...
  <div id="formSection" class="${not empty foundEmails ? 'hidden' : ''}">
    ...
    <form action="/find-id" method="post" class="space-y-5">
      ...
      <button type="submit" class="w-full bg-[#1B3A5F] text-white py-3 rounded-xl font-semibold hover:bg-[#2C5282] transition-colors shadow-lg">아이디 찾기</button>
    </form>
  </div>
  ...
  <div id="resultSection" class="${empty foundEmails ? 'hidden' : ''} space-y-6">
    ...
    <div class="flex gap-3">
      ...
      <a href="/find-password" class="flex-1 text-center border-2 border-[#1B3A5F] text-[#1B3A5F] py-3 rounded-xl font-semibold hover:bg-[#1B3A5F]/5 transition-colors">비밀번호 찾기</a>
    </div>
  </div>

  <div class="mt-6 pt-6 border-t border-gray-100 flex justify-center gap-6 text-sm">
    <a href="/find-password" class="text-gray-500 hover:text-[#FF6B35] transition-colors">비밀번호 찾기</a>
