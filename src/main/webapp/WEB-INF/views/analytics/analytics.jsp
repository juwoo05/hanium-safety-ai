  ...
  <header class="sticky top-0 z-20 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
    ...
    <div class="flex items-center gap-4 ml-4">
      <select class="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-[#FF6B35] outline-none">
        <option>?대쾲 ??/option><option>吏????/option><option>理쒓렐 3媛쒖썡</option><option>?ы빐</option>
      </select>
    ...
    <div class="space-y-6">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white rounded-2xl p-6 shadow-md">
          <div class="flex items-center justify-between mb-5">
            <h3 class="text-base font-semibold text-gray-900">?붾퀎 ?꾪뿕 媛먯? 異붿씠</h3>
            <div class="flex items-center gap-4 text-xs text-gray-500">
              <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-sm bg-[#FF6B35] inline-block"></span>怨좎쐞??/span>
              <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-sm bg-orange-400 inline-block"></span>以묒쐞??/span>
              <span class="flex items-center gap-1"><span class="w-3 h-3 rounded-sm bg-yellow-400 inline-block"></span>??꾪뿕</span>
            </div>
          </div>
          <%
          ...
          <div class="flex items-end gap-1.5 h-48 px-2">
            ...
            <div class="flex-1 flex flex-col gap-0.5 items-center group">
              <div class="w-full flex flex-col gap-0.5 items-stretch" style="height:180px;justify-content:flex-end">
                <div class="w-full bg-[#FF6B35] rounded-t" style="height:<%=hp%>%;min-height:2px" title="怨좎쐞??<%=highRisk[i]%>嫄?></div>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-md flex flex-col">
          <h3 class="text-base font-semibold text-gray-900 mb-5">?꾪뿕??遺꾪룷</h3>
          <div class="flex-1 flex flex-col items-center justify-center">
            ...
            <div class="mt-5 space-y-2 w-full">
              <div class="flex items-center justify-between text-sm"><span class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-[#FF6B35] inline-block"></span>怨좎쐞??/span><span class="font-bold text-gray-900">30% (103嫄?</span></div>
              <div class="flex items-center justify-between text-sm"><span class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-orange-400 inline-block"></span>以묒쐞??/span><span class="font-bold text-gray-900">22% (75嫄?</span></div>
              <div class="flex items-center justify-between text-sm"><span class="flex items-center gap-2"><span class="w-3 h-3 rounded-full bg-yellow-400 inline-block"></span>??꾪뿕</span><span class="font-bold text-gray-900">48% (164嫄?</span></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-md">
          <h3 class="text-base font-semibold text-gray-900 mb-5">?꾪뿕 ?좏삎蹂?遺꾨쪟</h3>
          <div class="space-y-4">
            <% String[] cats={"異붾씫 ?꾪뿕","?꾧린 ?꾪뿕","?붿옱 ?꾪뿕","?덉쟾?λ퉬 誘몄갑??,"援ъ“臾?遺덉븞??}; int[] catVals={45,28,18,35,22}; String[] catColors={"bg-red-500","bg-orange-500","bg-yellow-500","bg-blue-500","bg-purple-500"}; int catMax=45; %>
            <% for(int i=0;i<cats.length;i++){ int pct=catVals[i]*100/catMax; %>
            <div>
              <div class="flex justify-between text-sm mb-1"><span class="text-gray-700 font-medium"><%=cats[i]%></span><span class="font-bold text-gray-900"><%=catVals[i]%>嫄?/span></div>
        ...
        <!-- Company Ranking -->
        <div class="bg-white rounded-2xl p-6 shadow-md">
          <h3 class="text-base font-semibold text-gray-900 mb-5">?낆껜蹂?議곗튂 ?꾨즺??/h3>
          <div class="space-y-3">
            <% String[] companies={"(二??쒓뎅嫄댁꽕","??깆쿋怨?二?","誘몃옒?꾧린?ㅻ퉬","?덉쟾?뚰듃?덉뒪","?꾨??ㅻ퉬(二?"}; int[] completion={92,87,78,95,71}; String[] grades={"A","B+","B-","A+","C+"}; %>
      ...
      <div class="bg-gradient-to-r from-[#1B3A5F] to-[#2C5282] rounded-2xl p-6 text-white">
        <div class="flex items-start gap-4">
          ...
          <div>
            <h3 class="text-base font-bold mb-2">AI ?몄궗?댄듃 ???대쾲 ???듭떖 ?ъ씤??/h3>
            <ul class="space-y-1.5 text-white/80 text-sm">
              <li class="flex items-start gap-2"><span class="text-[#FF6B35] mt-0.5 flex-shrink-0">??/span>異붾씫 ?꾪뿕 媛먯? 嫄댁닔媛 ?꾩썡 ?鍮?15% 利앷??덉뒿?덈떎. 3???μ긽 ?묒뾽 ???밸퀎 二쇱쓽媛 ?꾩슂?⑸땲??</li>
              <li class="flex items-start gap-2"><span class="text-[#FF6B35] mt-0.5 flex-shrink-0">??/span>?덉쟾?뚰듃?덉뒪(95%)? (二??쒓뎅嫄댁꽕(92%)??議곗튂 ?꾨즺???곸쐞沅뚯엯?덈떎. ?곗닔 ?щ?瑜?怨듭쑀?섏꽭??</li>
              <li class="flex items-start gap-2"><span class="text-[#FF6B35] mt-0.5 flex-shrink-0">??/span>?됯퇏 議곗튂 湲곌컙??2.3?쇰줈 ?⑥텞?섏뿀?듬땲?? 吏?????鍮?8% 媛쒖꽑?섏뿀?듬땲??</li>
            </ul>
          </div>
        </div>
      </div>
